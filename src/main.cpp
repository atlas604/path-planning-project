      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
              ...
              if (lane == 0)
              {
                ...
                for(int i=0; i < sensor_fusion.size(); i++)
                {
                  ...
                  if(d < (2+4*(lane+1)+2) && d > (2+4*(lane+1)-2))
                  {
                    ...
                    // // predicts the future position of car_s
                    // check_car_s+=((double)prev_size*.02*check_speed);
            ...
            // to interpolate these waypoints with spline later
            ...
            else
            {
              ...
              car_speed = (sqrt((ref_x-ref_x_prev)*(ref_x-ref_x_prev)+(ref_y-ref_y_prev)*(ref_y-ref_y_prev))/.02)*2.23694;

              // use two points that make the path tangent to the previous path's end pointee_t
	    ...
	    vector<double> next_wp1 = getXY(car_s+50,(2+4*lane),map_waypoints_s,map_waypoints_x,map_waypoints_y);
            ...
            // fill rest of path planner after filling it with previous points, here will always output 50 points
            for (int i=1; i <= 50-previous_path_x.size(); i++)
            {
              ...
              // 2.24 converts miles/h to meters/s
